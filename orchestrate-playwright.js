...
const shellOption = process.platform === 'win32' ? 'powershell.exe' : true;

async function main() {
  // Inicia o backend
  const backend = spawn('npm', ['run', 'start:dev'], { cwd: './api', shell: shellOption, stdio: 'inherit' });
  ...
  try {
    ...
    const playwright = spawn('npx', ['playwright', 'test'], { cwd: './api', shell: shellOption, stdio: 'inherit' });
